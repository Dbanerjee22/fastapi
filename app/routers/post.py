...
def create_post(post: schema.PostCreate,db: Session = Depends(get_db),current_user:int = Depends(oauth2.get_current_user)):
    # print(post.content)
    # print(new_post.dict) convert request data to dictionary 
...
def delete_post(id : int,db: Session = Depends(get_db),current_user:int = Depends(oauth2.get_current_user)):
    ...
    if not post:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,detail=f"Unable to delete Post.No Post found with id:{id}")
    if post.owner_id != current_user:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN,detail=f"Unable to delete Post.User is not the current user")
...
def update_post(id: int,updated_post: schema.PostCreate,db: Session = Depends(get_db),current_user:int = Depends(oauth2.get_current_user)):
    ...
    if not post:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,detail=f"Unable to delete Post.No Post found with id:{id}")
    if post.owner_id != current_user:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN,detail=f"Unable to Update Post.User is not the current user")
    post.update(updated_post.dict(),synchronize_session=False)
    db.commit()
    return post
""""
    for index,post_item in enumerate(my_posts):
        if post_item['id']==id:
            post_dict=post.dict()
            post_dict['id']=id
